...
function ok(id: McpRequest["id"], result: unknown) {
  send({ jsonrpc: "2.0", id, result });
}

function err(id: McpRequest["id"], code: number, message: string) {
...
const TOOLS = [
  {
    name: "arboard_review",
    description:
      "Submit a Salesforce Solution Design Document (SDD) or architecture text to ARBoard's multi-agent review pipeline. Returns a structured review with findings, risk ratings, and ADR recommendations from specialist agents (Designer, LWC, OmniStudio, Flow, Apex, Patterns, Integration, Data, Agentforce).",
    inputSchema: {
      type: "object",
      properties: {
        content: {
...
async function handleReview(args: Record<string, unknown>): Promise<string> {
  const { content, title = "Untitled SDD", domain = "general" } = args as {
    content: string;
    title?: string;
    domain?: string;
  };
  ...
  if (ARBOARD_API_KEY) headers["x-api-key"] = ARBOARD_API_KEY;
...
async function handleHealth(): Promise<string> {
  try {
    ...
    if (res.ok) return "ARBoard is running and reachable.";
...
async function dispatch(req: McpRequest) {
  ...
  switch (method) {
    ...
    case "tools/call": {
      ...
      try {
        let text: string;
        if (name === "arboard_review") text = await handleReview(args);
        else if (name === "arboard_health") text = await handleHealth();
